/**
 * _geom_gantt_nospawn.cjs —— v0.5 个人主页「8 周资源占用甘特」几何 + 色值探针
 *                            （**无 spawn 版**：假定 Chrome 已在 DP_GEOM_PORT 上监听）
 * ============================================================================
 * 本文件是 tests/_geom_gantt.cjs 的副本，**只删掉「自己启动 Chrome」那一段**：
 *   原版用 child_process.spawn 起 Chrome —— 在本机「node 进程内 spawn 子进程」返回 EBUSY，
 *   故由 **Bash 编排器 tests/mutation.sh 先起 Chrome**，本探针只做 HTTP + WebSocket 的 CDP 采集。
 * 其余判定逻辑、阈值、EXPECT_TIERS、输出格式与退出码语义**与原件完全一致**：
 *   末行打印 `GEOM_JSON=` + JSON；ok=true → exit 0；ok=false → exit 2；连不上 Chrome → exit 1。
 *
 * 运行：由 mutation.sh 在「Bash 已起 Chrome（--remote-debugging-port=$DP_GEOM_PORT）」前提下调用。
 */
'use strict';
...
(async () => {
  /* ★ 唯一改动点：不再 spawn Chrome，改为等待「Bash 已起好的 Chrome」在 PORT 上出现 page target。 */
  let page = null;
  for (let i = 0; i < 80; i++) {
    try {
  ...
  if (!page) { console.error('未找到 page target（Chrome 未在 ' + PORT + ' 监听？）'); process.exit(1); }
