...
### 同一次事故的第二轮：提交仍被污染，已修正

清理第一条僵死进程、恢复 `PersonProfile.jsx` 并完成第一次提交（`a04369c`）之后，
又出现了一个**新的** `mutation.sh`（08:06:29 启动）。它在 `a04369c` 提交之后、
`2da2ae2` 提交之前注入了 `global.css` 变异（删掉 `@media` 内 `.dp-demand-promise { order: 1 }`），
于是 `2da2ae2`（docs 提交）**连带把被注入的 `global.css` 和变异期的 `dist` 一起提交了**。

它随后以**完全相同的方式僵死**（08:11 后再无产物输出）。处置：

| 步骤 | 动作 | 结果 |
|---|---|---|
| 1 | 终止 PID 1146 + 清理 9346 端口的 Chrome | 进程清空 |
| 2 | 以 `.mutbase` 逐文件比对 | 9 个文件中仅 `BrdAssistantCard.jsx` 漂移（被注入 `.filter((x) => !x.pending)`） |
| 3 | `git checkout a04369c -- <该文件>` + 复核 | `DIFF_COUNT=0`，9 个目标全部回到基线 |
| 4 | `rm -rf dist && vite build` + smoke | 构建通过，**smoke 209/209 EXIT=0** |
| 5 | 同步 `portal-site/`、重打 zip | 与 `dist` 逐字节一致，zip 无反斜杠条目 |
| 6 | 新增修正提交，并把 `v0.6` tag 移到该提交 | 见 §6.1 |

**僵死的真因（两次一致）**：探针用的是一个**固定端口 9346** 的 headless Chrome
（`--headless=new --remote-debugging-port=9346`，两次实测端口号相同）。
前一个 Chrome 实例没有退出时，后续实例无法正常启动/连接，脚本就此阻塞、不再有任何输出。
—— 先前把该端口记为「动态分配」是错的，已更正：**端口固定，因此「9346 上还有 Chrome」
可以直接当作残留判据**。

另外，两轮之间工作区多出 `portal/tests/.mutation.lock` 残留（脚本被强杀时不会自己清），
它会让下一次运行以为「已有实例在跑」。强杀之后需要手动删除该锁文件。
