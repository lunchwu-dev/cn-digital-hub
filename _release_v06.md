...
| 变异自证 | **25 条全部按预期变红**，0 未通过，收尾 sha256 全部回到基线 |
| 品牌区人工复核 | 放大 4× 裁切顶栏，确认图标读得出「被钉住的公告板」、字标与副标正确 |

> 关于 25 条怎么跑：本机宿主会在约 2–13 分钟处**回收后台进程**（实测两次整跑分别在 M6 / M20
> 被 SIGKILL，退出码 137）。因此 `tests/mutation.sh` 支持 `MUT_ONLY` 分块执行，
> 本次分 4 块跑完 25 条；每块都以「收尾 sha256 全部回到基线」结束。
> 被 SIGKILL 时 trap 不会执行，故**源码会留在变异态** —— 恢复方式：
> `node tests/_mut_restore.cjs all`（从运行前快照还原，不依赖 git）。
> 另一处返修：smoke 若**没跑起来**（无汇总行）曾被记成「断言未变红」，属误归因；
> 现已改为重试一次，仍无汇总行则明确记「smoke 未跑起来（结论未知）」。
